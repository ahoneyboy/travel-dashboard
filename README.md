...
| 愿望清单 | 想去 / 已去 / 放弃状态流转，标记已去有正反馈 Toast；**分享文案粘贴导入**（见下文） |
...
在小红书 / 抖音 / 微博看到想去的地方，三种方式带入应用（全部离线识别，无任何外部请求）：

1. **手动粘贴（通用）**：复制分享文案 → 愿望清单 →「粘贴导入」→ 智能识别 → 勾选导入
2. **Android 分享面板**：把应用安装到主屏幕（PWA）后，在任意 App 点分享 → 选「旅行工作台」，文案自动带入
3. **iOS 快捷指令**：快捷指令 App 新建一条——接收「共享表单」→「URL 编码」输入内容 → 拼接打开
   `https://ahoneyboy.github.io/travel-dashboard/#/wishlist?share=<编码后文本>`

识别逻辑（`src/utils/parse-share.js`）分两层：

- **内置景区库匹配**：文案提到库里 191 个景点（含「洱海」「茶卡」「天空之镜」等 50+ 个俗称别名）时，自动带出等级、最佳季节、图片等完整资料；
- **新地点启发式**：未收录的地点按地名后缀（古镇 / 岛屿 / 瀑布 / 盐湖 / 夜市…约 35 类）提取候选，并过滤「明天去」「附近」等时间方位修饰词；识别结果全部经过人工勾选确认后才入库。

> 平台限制说明：小红书等平台有登录墙和反爬机制，纯前端应用也无法跨域抓取网页，因此不做「自动爬取」；分享 → 识别 → 确认是最稳妥的路径。

## 数据与备份

- **存储位置**：浏览器 IndexedDB（数据库名 `travel-workbench`），关闭浏览器不会丢失。
- **演示数据**：首次打开自动写入「日本 · 关西之旅」等演示数据；在「设置 → 数据管理 → 清空数据」后不会再次写入。
- **景区预设库**：内置约 190 个国内热门景点 / 网红景点 / 4A-5A 景区（覆盖 31 省 + 港澳台，见 `src/data/attractions.js`），在「目的地库」按等级（5A / 4A / 网红 / 热门）筛选。数据带版本号：应用更新预设库后，老用户打开会自动**合并新增条目**（按名称去重，不覆盖你的去过状态 / 评分 / 笔记）。A级评定以文旅部公告为准，个别标注可能滞后，可在应用内修改。
